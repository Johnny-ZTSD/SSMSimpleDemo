...
import cn.johnnyzen.common.collection.CollectionUtil;
import cn.johnnyzen.common.date.DatetimeUtil;
...
public class RequestUtil {
    ...
    public static HashMap<String,Cookie> getCookiesFromLocalFile(boolean isOuputLogToConsole,String cookiesFilePath){
        Print.print(isOuputLogToConsole,"start execute cookies file '"+cookiesFilePath +"'.");
...
//        String cookiesFilePath = "C:\\Users\\千千寰宇\\Desktop\\cookies.txt";
        ...
        if(!cookiesFile.exists()){//IF 文件不存在
            LOGGER.error("Fail to invoke!Not found the file '"+cookiesFilePath+"'!");
        ...
        try {
            ...
            e.printStackTrace();
            LOGGER.error("Fail to invoke!Because: Fail to read first line data!");
            return cookies;//size:0
        }
        ...
        try {
            ...
            e.printStackTrace();
            LOGGER.warn("Fail to invoke!Because: Fail to read cookie data after first line!");
            return cookies;//size: 0 or other
        } catch (ParseException e) {//解析日期失败
            e.printStackTrace();
            LOGGER.warn("Fail to parse dateStr as Object 'expire' for 'DatetimeUtil.stringToDate'!");
            return cookies; //size: 0 or other
        }
        return cookies;//size: 0 or other
    }
    ...
    public static String findJavaScriptCodeOfHTMLByScriptFragment(String sourceHtml,String scriptFragment){
        if(sourceHtml==null ||scriptFragment==null){
            LOGGER.warn("sourceHtml or scriptFragment is null!");
            return null;
        } else {
